...
class BSEnvironment:
    def __init__(self, agents, deck, debug_config=None, log_dir=None):
        self.agents = agents
        self.deck = deck
        self.pile = []
    ...
    def _system_prompt(self):
        rules = [
            ...
            "4. Your opponent may Challenge if they think you lied.",
            "5. If challenged and lying: you pick up the pile.",
            "6. If challenged and truthful: challenger picks up the pile.",
            "7. Always respond in JSON with fields: Reasoning, Action, Declared_Rank, Card_idx.",
            "8. Action must be PLAY, Challenge, or Pass.",
            "9. Reasoning must come first."
        ]
        return "You are playing the game of BS.\nRules:\n" + "\n".join(rules)
...
"""
    ...
    @classmethod
    def from_snapshot(cls, snapshot, agents, deck, debug_config=None, log_dir=None):
        env = cls(agents, deck, debug_config=debug_config, log_dir=log_dir)
